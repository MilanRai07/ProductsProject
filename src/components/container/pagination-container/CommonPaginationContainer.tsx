...
const CommonPaginationContainer: React.FC<Props> = ({ assetsArray, onPageChange, isFetching, isError, goTop }) => {

    const pagination = assetsArray?.pagination ?? { totalPages: 1, currentPage: 1, limit: 2 };
            ...
            {isFetching ? (
                ...
                assetsArray?.items.length == 0 ? (
                    ...
                    assetsArray == undefined ? (
                        ...
                        >
                            <div className="flex flex-wrap justify-center gl6:grid gl6:grid-cols-2 gl6:gap-4 as1:grid-cols-1 grid-rows-auto gap-4
                          p-4 gl6:h-auto w-full " >
                                {currentPageData?.map((asset: ProductTypes) => {
                                ...
                                Array.from({ length: 8 }).map((_, index) => (
                                    <AssetsCard key={index} image="https://placehold.co/300" title={`Card ${index + 1}`} />
                                ))
                            } */}
                            </div>
